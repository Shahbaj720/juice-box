...
async function deletePost(postId) {
  try {
    // Perform a SQL DELETE operation to delete the post by its ID
    ...
    // Check if a row was deleted (indicating successful deletion)
    if (result.rowCount === 1) {
      // Return the deleted post
      return result.rows[0];
    } else {
      // If no rows were deleted, the post with the given ID doesn't exist
